...
public class WebServer extends NanoHTTPD {
    ...
    public WebServer(android.content.Context ctx) throws java.io.IOException {
        super(5000);
        this.ctx = ctx;
    ...
    public Response serve(IHTTPSession session) {
        ...
        String qApiKey = session.getParms().getOrDefault("api_key", "");
        boolean auth  = gw.apiKey.equals(apiKey) || gw.apiKey.equals(qApiKey);

        try {
            // Static pages
            if (uri.equals("/") || uri.equals("/index.html")) return serveAsset("admin.html", "text/html");
            if (uri.startsWith("/pay/")) return serveAsset("pay.html", "text/html");

            // Public
            if (uri.startsWith("/api/order/") && uri.endsWith("/utr") && method.equals("POST")) {
                String oid = uri.split("/")[3].toUpperCase();
                return handleSubmitUtr(oid, session);
            }
            if (uri.startsWith("/api/order/") && method.equals("GET")) {
                String oid = uri.split("/")[3].toUpperCase();
            ...
            if (uri.equals("/api/create_order") && method.equals("POST")) return handleCreateOrder(session);
            if (uri.equals("/api/orders")) return handleListOrders();
            if (uri.startsWith("/api/cancel/") && method.equals("POST")) {
                String oid = uri.split("/")[3].toUpperCase();
                return handleCancel(oid);
            }
            if (uri.startsWith("/api/order/") && uri.endsWith("/verify_utr") && method.equals("POST")) {
                String oid = uri.split("/")[3].toUpperCase();
                return handleVerifyUtr(oid, session);
            }
            if (uri.equals("/api/stats")) return jsonResp(new JSONObject(gw.getStats()).toString());
            if (uri.equals("/api/config") && method.equals("GET")) {
                JSONObject c = new JSONObject();
                c.put("upi_id", gw.upiId); c.put("upi_name", gw.upiName);
                return jsonResp(c.toString());
            }
            if (uri.equals("/api/config") && method.equals("POST")) return handleUpdateConfig(session);
    ...
    private Response handleCreateOrder(IHTTPSession s) throws Exception {
        ...
        r.put("pay_url", host + "/pay/" + o.get("order_id"));
    ...
    private Response handleSubmitUtr(String oid, IHTTPSession s) throws Exception {
        Map<String,Object> o = gw.orders.get(oid);
        if (o == null) return jsonResp("{\"error\":\"not found\"}", Response.Status.NOT_FOUND);
        if ("SUCCESS".equals(o.get("status"))) return jsonResp("{\"status\":\"SUCCESS\"}");
        JSONObject body = parseBody(s);
        String utr = body.optString("utr","").trim();
        if (utr.length() < 6) return jsonResp("{\"error\":\"Invalid UTR\"}", Response.Status.BAD_REQUEST);
        o.put("utr", utr); o.put("utr_status", "PENDING_VERIFY");
        return jsonResp("{\"status\":\"ok\",\"message\":\"UTR submitted\"}");
    }
    ...
    private Response handleVerifyUtr(String oid, IHTTPSession s) throws Exception {
        ...
        if ("approve".equals(action)) {
            o.put("status","SUCCESS"); o.put("utr_status","VERIFIED");
            o.put("paid_by","UTR:"+o.get("utr"));
            o.put("paid_at", new java.util.Date().toString());
            gw.releasePenny((double)o.get("unique_amount"));
        } else { o.put("utr_status","REJECTED"); }
        return jsonResp(mapToJson(o).toString());
    }
